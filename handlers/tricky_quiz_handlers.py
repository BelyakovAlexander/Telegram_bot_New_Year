...
from keyboards import coal_or_gift_kbd, coal_or_gift_keyboard_inline, next_question_or_quit_inline
...
@user_router.callback_query(StateFilter(None), F.data == 'judge')                                      # TODO (с этим декоратором бот отвечает не сообщением, а всплывающим окном с тем же текстом)
async def bot_quiz(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(f'Давай сыграем в игру <b>"Какой ты Дед Мороз"</b>: '
                         ...
                         f'что заслужил этот ребенок в этом году: подарок или уголек.',
                         reply_markup=next_question_or_quit_inline)
...
@user_router.callback_query(QuizStates.behavior_quiz_state, F.data == 'more')               # test if F.data not defined - will it work?
...
@user_router.callback_query(QuizStates.quiz_question, F.data == ('coal', 'gift', 'more'))
...
async def quiz_question(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if children_actions[res['question']] == 'coal':
        if usr_answer.lower() == 'gift':
            ...
            await callback.message.answer(f'Ты добряк')

    elif children_actions[res['question']] == 'gift':
        if usr_answer.lower() == 'coal':
            await state.update_data(user_rating=res['user_rating'] - 1)
            await callback.message.answer(f'Ты злодей')
    await callback.message.answer(f'Рейтинг: {res["user_rating"]}\tВопрос № {res["question_num"]}')

    if res['question_num'] <= 4:
        await callback.message.answer('Следующий вопрос?', reply_markup=next_question_or_quit_inline)
        await state.set_state(QuizStates.behavior_quiz_state)
        # await quiz_question
    else:
        await callback.message.answer('Закончили упражнение. Хочешь узнать результаты?')
